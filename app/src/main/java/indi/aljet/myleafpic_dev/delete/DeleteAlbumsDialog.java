...
public class DeleteAlbumsDialog extends DialogFragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        t = ThemeHelper.getInstance(getActivity());
        ArrayList<Album> asd = getArguments()
                .getParcelableArrayList("albums");
        if(asd != null){
            for(Album album : asd){
                albums.add(new Folder(album
                .getPath(),album.getCount()));
            }
        }
        setCancelable(false);
    }
    ...
    class DeleteAlbums extends AsyncTask<String,
            Integer,Boolean>{
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ThemedAlertDialogBuilder builder = new
                ThemedAlertDialogBuilder(getActivity(),
                ThemeHelper.getInstanceLoaded(getContext()));
        View view = LayoutInflater.from(getContext())
                .inflate(R.layout.dialog_delete_album_progress,
                        null,false);

        RecyclerView rv = (RecyclerView) view.findViewById(R.id.rv_folders);
        adapter = new FolderAdapter(getContext(),
                albums);
        rv.setLayoutManager(new LinearLayoutManager(getContext()));
        rv.setAdapter(adapter);

        dialogTitle = (TextView) view.findViewById(R.id.text_dialog_title);
        fileName = (TextView)view.findViewById(R.id.file_name);
        progress = (ProgressBar) view.findViewById(R.id.progress_bar);

        ((CardView)view.findViewById(R.id.message_card)).setCardBackgroundColor(t
