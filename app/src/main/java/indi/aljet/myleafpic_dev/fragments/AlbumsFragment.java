...
import indi.aljet.myleafpic_dev.adapters.AlbumsAdapter;
...
import indi.aljet.myleafpic_dev.delete.DeleteAlbumsDialog;
...
public class AlbumsFragment extends BaseFragment {
    ...
    ArrayList<String> excused = new ArrayList<>();


    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        excused = db().getExcludedFolders(getContext());
    ...
    private void displayAlbums(){
        ...
        CPHelper.getAlbums(getContext(),
                hidden, excused,sortingMode(),
                sortingOrder())
                .subscribeOn(Schedulers.io())
                .map(album ->  album.withSettings(
                db().getSettings(album.getPath())))
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(album -> adapter.add(album)
                        ,
                        throwable -> {
...
//                            "albums",adapter.getAlbumsPaths());
    ...
    public void setUpColumns(){
        ...
        .getLayoutManager()).getSpanCount()){
            ...
            spacingDecoration = new GridSpacingItemDecoration
                    (columnsCount, Measure.pxToDp(3,getContext()),true);
    ...
    private int columnsCount(){
        return getResources().getConfiguration()
                ...
                .ORIENTATION_PORTRAIT ? Hawk
                .get("n_columns_folders",2) :
    ...
    private void updateToolbar(){
        if(editMode()){
            act.updateToolbar(String.format(Locale.ENGLISH,
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable
        ...
        View v = inflater.inflate(R.layout.fragment_albums,
                null);
        ButterKnife.bind(this,v);
        int spanCount = columnsCount();
        spacingDecoration = new GridSpacingItemDecoration
                (spanCount,Measure.pxToDp(3,getContext()),true);
        ...
        adapter = new AlbumsAdapter(getContext(),
                sortingMode(),
                sortingOrder());
        Log.d("AlbumsAdapter:" ,"Count = " + adapter.getItemCount());

        adapter.getClicks().subscribeOn(Schedulers
        .newThread()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(album -> act.displayMedia(album));

        adapter.getSelectedClicks().subscribeOn(
                Schedulers.newThread()).observeOn(AndroidSchedulers
    ...
    public SortingMode sortingMode(){
        return adapter != null ? adapter.sortingMode() :
                AlbumsHelper.getSortingMode(getContext());
    }

    public SortingOrder sortingOrder(){
        return adapter != null ? adapter.sortingOrder():
    ...
    private AlbumsSqlData db(){
        return AlbumsSqlData.getInstance(getContext());
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        menu.findItem(R.id.select_all).setIcon(ThemeHelper
        .getToolbarIcon(getContext(),GoogleMaterial.Icon
        .gmd_select_all));
        menu.findItem(R.id.delete).setIcon(ThemeHelper
        .getToolbarIcon(getContext(),(GoogleMaterial.Icon
        .gmd_delete)));
        menu.findItem(R.id.sort_action).setIcon(ThemeHelper
        .getToolbarIcon(getContext(),(GoogleMaterial
        .Icon.gmd_sort)));
        menu.findItem(R.id.search_action).setIcon(ThemeHelper.getToolbarIcon
                (getContext(),(GoogleMaterial.Icon.gmd_search)));
    }
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        menu.findItem(R.id.select_all).setTitle(getSelectedCount() ==
        getCount() ? R.string.clear_selected :
                R.string.clear_selected);
        if(!editMode){
            menu.findItem(R.id.ascending_sort_order).setChecked(sortingOrder()
        ...
        if(oneSelected){
            Album selectedAlbum = adapter
                    .getFirstSelectedAlbum();
            menu.findItem(R.id.pin_album).setTitle(selectedAlbum
                    .isPinned() ? getString(R.string.un_pin) :
                    getString(R.string.pin));
            menu.findItem(R.id.clear_album_cover).setVisible(selectedAlbum
            .hashCover());
        }
    }
        ...
        switch (item.getItemId()){
            ...
            case R.id.pin_album:
                if(selectedAlbum != null){
                    boolean b = selectedAlbum
                            .togglePinAlbum();
                    db().setPined(selectedAlbum
                    .getPath(),b);
                    adapter.clearSelected();
                    adapter.sort();
                }
            ...
            case R.id.clear_album_cover:
                ...
                return false;
            case R.id.shortcut:
                AlbumsHelper.createShortcuts(getContext(),
                        adapter
                .getSelectedAlbums());
                adapter.clearSelected();
                return true;
            case R.id.name_sort_mode:
                adapter.changeSortingMode(
                        SortingMode.NAME);
                AlbumsHelper.setSortingMode(
                        getContext(),SortingMode.NAME);
                item.setCheckable(true);
                return true;
            case R.id.date_taken_sort_mode:
                adapter.changeSortingMode(SortingMode.DATE);
                AlbumsHelper.setSortingMode(
                        getContext(),SortingMode.DATE);
                item.setChecked(true);
                return true;
            case R.id.size_sort_mode:
                adapter.changeSortingMode(SortingMode
                .SIZE);
            ...
            case R.id.ascending_sort_order:
                ...
                return true;
            case R.id.exclude:
                final AlertDialog.Builder excludeDialongBuilder =
                        new AlertDialog.Builder(getActivity(),
                                getDialogStyle());

                final View excludeDialogLayout = LayoutInflater
                        .from(getContext())
                        .inflate(R.layout.dialog_exclude,null);
                ...
                spinnerParents.getBackground().setColorFilter(getIconColor(),
                ...
                if(adapter.getSelectedCount() > 1){
                    textViewExcludeMessage.setText(R.string
                    .exclude_album_message);
                    spinnerParents.setVisibility(View.GONE);
                }else{
                    textViewExcludeMessage.setText(R.string
                    .exclude_album_message);
                    spinnerParents.setAdapter(getThemeHelper()
                    .getSpinnerAdapter(adapter
                    .getFirstSelectedAlbum().getParentsFolders()));
                }
                textViewExcludeMessage.setTextColor(getTextColor());
                excludeDialongBuilder.setView(excludeDialogLayout);

                excludeDialongBuilder.setPositiveButton(this.getString(
                        R.string.exclude).toUpperCase(),(dialog,id)
                -> {
                    if(adapter.getSelectedCount() > 1){
                        for( Album album : adapter.getSelectedAlbums()){
                            db().excludeAlbum(album);
                            excused.add(album.getPath());
                        }
                    }else{
                        String path = spinnerParents.getSelectedItem()
                                .toString();
                        Album a = new Album(path,"");
                        db().excludeAlbum(a);
                ...
                excludeDialongBuilder
                        .setNegativeButton(this.getString(R
                        .string.cancel).toUpperCase(),null);
                excludeDialongBuilder.show();
                return true;
            case R.id.delete:
