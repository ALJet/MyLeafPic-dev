...
public class Affix {
    ...
    public static void AffixBitmapList(Context ctx,
                                       ArrayList<Bitmap> bitmapArray,
                                       Options options){
        Bitmap unionBitmap;
        if(options.isVertical()){
            unionBitmap = Bitmap.
                    createBitmap(getMaxBitmapWidth(bitmapArray),
                            getMaxBitmapHeight(bitmapArray)
                            ,Bitmap.Config.ARGB_8888);
        }else{
            unionBitmap = Bitmap.createBitmap(getBitmapsWidth(bitmapArray)
            ,getMaxBitmapHeight(bitmapArray), Bitmap.Config.ARGB_8888);
        }
        Canvas comboImage = new Canvas(unionBitmap);

        combineBitmap(comboImage,bitmapArray,
                options.isVertical());
        saveFile(ctx,unionBitmap,options);
    }

    private static Canvas combineBitmap(Canvas cs,
                                        ArrayList<Bitmap> bpmList,
                                        boolean vertical){
        if(vertical){
            int height = bpmList.get(0).getHeight();
            cs.drawBitmap(bpmList.get(0),0f,0f,
                    null);
            for(int i = 1;i < bpmList.size();i++){
                cs.drawBitmap(bpmList.get(i),
                        0f,height,null);
                height += bpmList.get(i)
                        .getHeight();
            }
            return cs;
        }else{
    ...
    private static void saveFile(Context context,
        ...
        try{
            ...
            .currentTimeMillis() + "." + options
            .getExtensionFormat());
            if(file.createNewFile()){
                OutputStream os = new
                        FileOutputStream(file);
                bmp.compress(options.getFormat()
                ,options.getQuality(),os);
                os.close();
    ...
    private static int getBitmapsWidth(ArrayList<Bitmap>
                                       bpmHeightArray){
        int width = 0;
    ...
    public static class Options{
        ...
        public Options(String folderPath, Bitmap.CompressFormat format, int quality, boolean
                vertical) {
