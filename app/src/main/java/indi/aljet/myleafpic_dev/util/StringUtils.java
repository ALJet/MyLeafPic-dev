...
public class StringUtils {
    ...
    public static String getMimeType(String path){
        int index;
        if(path == null || (index = path.lastIndexOf('.'))
                == -1){
            return "unknown";
        }
        String mime = MimeTypeMap.getSingleton()
                .getMimeTypeFromExtension(path.substring(index
                + 1).toLowerCase());
    ...
    public static String join(String jointChar,
                              Object... collection){
        String s = "";
        for(Object o : collection){
            s += o.toString() + jointChar;
        }
    ...
    public static String getPhotoPathRenamed(String olderPath,
                                             String newName){
    ...
    public static String incrementFileNameSuffix(String name){
        ...
        String baseName = dot != -1 ? name
                .subSequence(0,dot).toString() : name;
        String nameWoSuffix = baseName;
        Matcher matcher = Pattern.compile("_\\d")
                .matcher(baseName);
        if(matcher.find()){
            int i = baseName.lastIndexOf("_");
            if(i != -1){
                nameWoSuffix = baseName.substring(0,i).toString();
            }
        }
        builder.append(nameWoSuffix).append("_").
        append(new Date().getTime());
        builder.append(name.substring(dot));
        return builder.toString();
    }

    public static String getPhotoPathRenamedAlbumChange(String
                                                        olderPath,
                                                        String
                                                        albumNewName){
        String c = "",b[] = olderPath.split("/");
    ...
    public static String getAlbumPathPathRenamed(String olderPath,
                                                 String newName){
        return olderPath.substring(0,olderPath
        .lastIndexOf('/')) + "/" + newName;
    }

    public static String getPhotoPathMoved(String olderPath,
                                           String folderPath){
    ...
    public static Spanned htmFormat(String content,
